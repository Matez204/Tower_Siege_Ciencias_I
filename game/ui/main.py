...
from bridge import Bridge, Pos, InputState, Tower, Castle, Horde
...
def reset_juego():
    ...
    global fase, ruta_visual, greedy_celda, estado_actual
    torres_construidas = {}
    celda_seleccionada = None
    current_tick       = 0
    oro_jugador        = ORO_INICIAL
        ...
        if evento.type == pygame.QUIT:
            ...
            # Engine tick
                ...
                try:
                    ...
                    if success:
                        ...
                        set_log(f"Tick {current_tick} procesado. HP: {hp_castillo}/{HP_MAX}", C_VERDE)
                    else:
            ...
            if evento.key == pygame.K_t and celda_seleccionada:
                ...
                if (f, c) in (POS_SPAWN, POS_CASTILLO):
                    ...
                    recalcular_ruta()
                    if ruta_visual:
                        set_log(f"Torre en ({f},{c}). Enemigos buscan ruta: {len(ruta_visual)} pasos.", C_AZUL)
                    else:
                        set_log(f"Torre en ({f},{c}). ¡Paso bloqueado completamente!", C_ROJO)
                else:
